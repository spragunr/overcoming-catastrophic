...
class Model: 
    def __init__(self, x, y_, weights = 50):
        ...
        self.expandable_weights = [[self.W1], [self.b1], [self.W2], [self.b2]]
    ...
    def expand(self, x, y_, sess, weights):
        ...
        for index in range(len(self.expandable_weights) - 1):
            self.expandable_weights[index].append(new_vars[index])
        
        self.expandable_weights[0] = [tf.concat(self.expandable_weights[0], 1)]
        self.expandable_weights[1] = [tf.concat(self.expandable_weights[1], 0)]
        self.expandable_weights[2] = [tf.concat(self.expandable_weights[2], 0)]
        
        self.h1.append(tf.nn.relu(tf.matmul(self.x,self.expandable_weights[0][0]) + self.expandable_weights[1][0]))
        self.y.append(tf.matmul(self.h1[len(self.h1) - 1], self.expandable_weights[2][0]) + self.expandable_weights[3][0])    
