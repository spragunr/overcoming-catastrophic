...
def train_task(model, sess, num_iter, disp_freq, trainset, testsets, x, y_, acc_sgd, 
    ...
    plot1.set_title("layers: {}  weights per layer: {}  lambda: {}  percent permutation: {}".format(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4]))
    ...
    if hasattr(model, "F_accum"):
        ...
        plot2.set_title("layers: {}  weights per layer: {}  lambda: {}  percent permutation: {}".format(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4]))
...
for i in range(25):
    ...
    if tasks == 1:
        ...
        train_task(model, sess, 800, 20, mnist, mnistList, x, y_, acc_sgd, acc_ewc, final_means, figNum, F_archives, tasks, lams=lambdas, expanding=True)
        cur_weights *= 2
    else:
        train_task(model, sess, 800, 20, mnist, mnistList, x, y_, acc_sgd, acc_ewc, final_means, figNum, F_archives, tasks, lams=lambdas)
    model.compute_fisher(mnist.validation.images, sess, F_archives, num_samples=200)
    model.star()
    mnist = permute_mnist_random(mnist, float(float(sys.argv[4])/100.0))
